# VirtEngine US-East-1 Regional Configuration (Primary)
# This is the primary region for the VirtEngine multi-region deployment
...
module "eks" {
  ...
  node_groups = {
    system = {
      ...
      labels = {
        role                    = "system"
        "virtengine.com/region" = local.region
      }
      taints = []
    }
    validators = {
...
# -----------------------------------------------------------------------------
# VPC Peering to Other Regions
# -----------------------------------------------------------------------------
resource "aws_vpc_peering_connection" "eu_west" {
  count = var.enable_cross_region_peering ? 1 : 0

  vpc_id      = module.vpc.vpc_id
  peer_vpc_id = var.eu_west_vpc_id
  peer_region = "eu-west-1"
  auto_accept = false

  tags = {
    Name = "virtengine-us-east-1-to-eu-west-1"
  }
}

resource "aws_vpc_peering_connection" "ap_southeast" {
  count = var.enable_cross_region_peering ? 1 : 0

  vpc_id      = module.vpc.vpc_id
  peer_vpc_id = var.ap_southeast_vpc_id
  peer_region = "ap-southeast-1"
  auto_accept = false

  tags = {
    Name = "virtengine-us-east-1-to-ap-southeast-1"
  }
}
