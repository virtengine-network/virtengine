#!/usr/bin/env pwsh
# Script to consolidate orchestrator/kanban to openfleet and remove Copilot PR notification code
# Run from repo root
...
$envPath = "$repoRoot/scripts/openfleet/.env"
...
$tokens = $null
$errors = $null
$orchestratorPath = Join-Path $PSScriptRoot 'openfleet\ve-orchestrator.ps1'
...
. $kanbanPath'), '$kanbanPath = Join-Path $repoRoot "scripts\openfleet\ve-kanban.ps1"
if (-not (Test-Path -LiteralPath $kanbanPath)) {
...
$configPath = "$repoRoot/scripts/openfleet/config.mjs"
$configContent = Get-Content $configPath -Raw
$configContent = $configContent -replace [regex]::Escape('// Default to sibling location (most common for npm-installed openfleet)
  return resolve(configDir, "..", "ve-orchestrator.ps1");'), '// Default to bundled location (inside openfleet dir)
...
$tgPath = "$repoRoot/scripts/openfleet/telegram-bot.mjs"
$tgContent = Get-Content $tgPath -Raw
$tgContent = $tgContent -replace [regex]::Escape('resolve(repoRoot, "scripts", "ve-kanban.ps1")'), 'resolve(repoRoot, "scripts", "openfleet", "ve-kanban.ps1")'
Set-Content $tgPath $tgContent -NoNewline
Write-Host "Updated telegram-bot.mjs"

Write-Host "`n=== Step 7: Update autofix.mjs ==="
$afPath = "$repoRoot/scripts/openfleet/autofix.mjs"
$afContent = Get-Content $afPath -Raw
$afContent = $afContent -replace [regex]::Escape('scripts/ve-orchestrator.ps1'), 'scripts/openfleet/ve-orchestrator.ps1'
Set-Content $afPath $afContent -NoNewline
Write-Host "Updated autofix.mjs"

Write-Host "`n=== Step 8: Update codex-shell.mjs ==="
$csPath = "$repoRoot/scripts/openfleet/codex-shell.mjs"
$csContent = Get-Content $csPath -Raw
$csContent = $csContent -replace [regex]::Escape('scripts/ve-orchestrator.ps1'), 'scripts/openfleet/ve-orchestrator.ps1'
Set-Content $csPath $csContent -NoNewline
Write-Host "Updated codex-shell.mjs"

Write-Host "`n=== Step 9: Update copilot-shell.mjs ==="
$cpPath = "$repoRoot/scripts/openfleet/copilot-shell.mjs"
$cpContent = Get-Content $cpPath -Raw
$cpContent = $cpContent -replace [regex]::Escape('scripts/ve-orchestrator.ps1'), 'scripts/openfleet/ve-orchestrator.ps1'
...
$agentsContent = $agentsContent -replace [regex]::Escape('scripts/ve-orchestrator.ps1'), 'scripts/openfleet/ve-orchestrator.ps1'
...
if (Test-Path $guidePath) {
    ...
    $guideContent = $guideContent -replace 'scripts/ve-orchestrator\.ps1', 'scripts/openfleet/ve-orchestrator.ps1'
    $guideContent = $guideContent -replace 'scripts/ve-kanban\.ps1', 'scripts/openfleet/ve-kanban.ps1'
    $guideContent = $guideContent -replace 'scripts\\ve-kanban\.ps1', 'scripts\\openfleet\\ve-kanban.ps1'
    $guideContent = $guideContent -replace 'scripts\\ve-orchestrator\.ps1', 'scripts\\openfleet\\ve-orchestrator.ps1'
