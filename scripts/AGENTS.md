...
| scripts/_consolidate-refs.ps1 | One-off migration to bosun paths. | pwsh scripts/_consolidate-refs.ps1 | PowerShell, git |
...
- Permissions: VK API access through ve-kanban.ps1 (see bosun docs).
...
- Purpose: Syntax-check PS1 files; defaults to bosun wrappers.
- Parameters: -Path (array of PS1 paths).
- Errors: exits 1 if any parse errors found.
- Example: pwsh scripts/_check-ps1-syntax.ps1 -Path scripts/bosun/ve-kanban.ps1
